...
#include "stdio.h"
#include "stdlib.h"
#include "Typedefs.h"  // Self-Defined Header file
// #include "stdint.h" // can also be used instead of the defined header file
...
int main()
{
	...
	while(1){
		...
		/* 1st way is the most advanced since we can write on the entire port
   or a specific pin if prefered, In this method a nested struct
   within a union has to be defined first then a union pointer
   has to point to the entire register */

		//		ODR->Pins.Pin13 = 1;     // Writing 1 on Struct member Pin13 to toggle led
		...
		// 2nd Way is By using a Pointer to Union,  Pointing to the entire Register Instead of a single bit

		//ODR->AllPort = 0x00;    // Writing on the entire port
		//for( i = 0; i<5000; i++); // Delay
		//ODR->AllPort = 0x00;    // Writing on the entire port
		//for( i = 0; i<5000; i++); // Delay again
   ...
   write into the register's desired pin as shown */
		...
		GPIO_PA_ODR &= 0x00;      // Clearing the register first from the last state
		GPIO_PA_ODR |= 1<<13;     // Shifting 13 units to the left for setting pin13 with 1
		for( i = 0; i<5000; i++); // Delay
		GPIO_PA_ODR &= ~(1<<13);  // Clearing pin 13
		for( i = 0; i<5000; i++); // Delay again
